...
import { resolvePublicInvoicePdfBuffer, resolvePublicQuotePdfBuffer } from "@/modules/public/pdf";
...
export async function GET(
  ...
  const file =
    params.type === "factures"
      ? await resolvePublicInvoicePdfBuffer(db, storage, params.token)
      : await resolvePublicQuotePdfBuffer(db, storage, params.token);
  if (!file) {
    return NextResponse.json({ error: "Introuvable" }, { status: 404 });
  }
